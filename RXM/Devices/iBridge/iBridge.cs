...
using static RXM.Devices.iBridge.MusicDB;


// This was terrible. This IS terrible. This code sucks but blame russound for their AMAZING documentation and really good working pdf files that you cant find anymore
// This is not finished or polished or anything really, its just an example to see if i could get this to work
namespace RXM.Devices.iBridge
{
    public enum ScreenType : byte
    {
        IPOD_NOW_PLAYING = 0x1E,
        IPOD_REQUEST = 0x1F,
        IPOD_REQUEST_PLAYLIST = 0x20,
        IPOD_REQUEST_GENRE = 0x21,
        IPOD_REQUEST_ARTIST = 0x22,
        IPOD_REQUEST_ALBUM = 0x23,
        IPOD_REQUEST_SONG = 0x24,
        IPOD_PLAYLIST_OPTIONS = 0x25,
        IPOD_GENRE_OPTIONS = 0x27,
        IPOD_ARTIST_OPTIONS = 0x28,
        IPOD_ALBUM_OPTIONS = 0x29
    }
    ...
    public static class RNETiBridge
    {
        ...
        // FUCK RNET I HATE IT. Had to get my iBridge and see what it sends for that. Pdf had missing bytes for some reason. Fuck rnet
        ...
        public static byte[] CreateNP(int source, int Id, string text)
        {
            ...
            payload.Add((byte)ScreenType.IPOD_NOW_PLAYING);
            payload.AddRange(TextBytes);
    ...
    public sealed class iBridge : IDisposable
    {
        ...
        private ScreenType CurrentScreen = ScreenType.IPOD_NOW_PLAYING;
        ...
        private enum SongScope { AllSongs, ArtistSongs, AlbumSongs, PlaylistSongs }
        private SongScope CurrentSongScope = SongScope.AllSongs;
        private List<TrackRef> VisibleSongs = new();

        // playlist stuff
        private readonly Dictionary<string, PlaylistDef> LocalPlaylists = new(StringComparer.OrdinalIgnoreCase);
        private PlaylistDef? CurrentLocalPlaylist;
        ...
        private string PlaylistsRoot =>
            Path.Combine(AppContext.BaseDirectory, "iBridge", "Playlists");

        private sealed record NavState(ScreenType Screen, int MenuOffset, string? CurrentArtist, string? CurrentAlbum, string? CurrentPlaylist, string? CurrentGenre, SongScope Scope, string? LocalPlaylistName);

        public iBridge(SerialConnection serial, int source)
        {
            _sm = serial;
            Source = source;
        }
        ...
        public async Task StartAsync(bool StartNP = true)
        {
            ...
            {
                throw new InvalidOperationException($"[NP iBridge] Couldn't connect to '{NPApi}':", ex);
            }
        ...
        private void SetNP()
        {
            CurrentScreen = ScreenType.IPOD_NOW_PLAYING;
            OffsetMENU = 0;
            Nav.Clear();
        ...
        public void HandlePress(RXM.Serial.Button e)
        {
            ...
            if (e.EventId == 0x27)
            {
                // No clue why, but sending the same now playing data just makes it not appear on the screen?
                // We have to resend that data with an extra space at the end of a random field to actually get it to show again
                // Stupid hack but it works well soooo its fine
            ...
            {
                if (CurrentScreen == ScreenType.IPOD_NOW_PLAYING)
                {
                    PushState();
                    OffsetMENU = 0;
                    DisplayRequest();
        ...
        {
            Nav.Push(new NavState(CurrentScreen, OffsetMENU, MusicDB.CurrentArtist, MusicDB.CurrentAlbum, MusicDB.CurrentPlaylist, MusicDB.CurrentGenre, CurrentSongScope, CurrentLocalPlaylist?.Name));
        }
        ...
        private void HandleBack()
        {
            ...
            switch (prev.Screen)
            {
                case ScreenType.IPOD_NOW_PLAYING: DisplayNowPlaying(); break;
                case ScreenType.IPOD_REQUEST: DisplayRequest(); break;
                case ScreenType.IPOD_REQUEST_PLAYLIST: DisplayPlaylists(); break;
                case ScreenType.IPOD_REQUEST_GENRE: DisplayGenres(); break;
                case ScreenType.IPOD_REQUEST_ARTIST: DisplayArtists(); break;
                case ScreenType.IPOD_REQUEST_ALBUM: DisplayAlbums(MusicDB.CurrentArtist); break;
                case ScreenType.IPOD_REQUEST_SONG: DisplaySongOpts(MusicDB.CurrentAlbum); break;
                case ScreenType.IPOD_PLAYLIST_OPTIONS: DisplayPlaylistOpts(MusicDB.CurrentPlaylist ?? ""); break;
                case ScreenType.IPOD_GENRE_OPTIONS: DisplayGenreOpts(MusicDB.CurrentGenre ?? ""); break;
                case ScreenType.IPOD_ARTIST_OPTIONS: DisplayArtistOpts(MusicDB.CurrentArtist ?? ""); break;
                case ScreenType.IPOD_ALBUM_OPTIONS: DisplayAlbumOpts(MusicDB.CurrentAlbum ?? ""); break;
        ...
        private static void TaskForget(Func<Task> fn)
        {
            ...
            {
                try { await fn(); }
                catch (Exception ex) { Console.WriteLine($"[iBridge] API error: {ex.Message}"); }
            });
        }
        ...
        {
            ScreenType.IPOD_REQUEST_SONG => 5,
            _ => 4
        };
        ...
        private void HandleNext()
        {
            ...
            {
                ScreenType.IPOD_REQUEST_PLAYLIST => MusicDB.Playlists.Count,
                ScreenType.IPOD_REQUEST_GENRE => MusicDB.Genres.Count,
                ScreenType.IPOD_REQUEST_ARTIST => MusicDB.Artists.Count,
                ScreenType.IPOD_REQUEST_ALBUM => GetAlbumsForArtist().Count,
                ScreenType.IPOD_REQUEST_SONG => VisibleSongs.Count,
                _ => 0
            };
        ...
        private void LoadLocalPlaylists()
        {
            ...
            foreach (var Dir in Directory.GetDirectories(PlaylistsRoot))
            {
                ...
                try
                {
                    ...
                    {
                        var tracks = JsonSerializer.Deserialize<List<TrackRef>>(JSON, _jsonOpts) ?? new();
                        pl = new PlaylistDef { Name = Folder, Tracks = tracks };
                    }
                    else
                    {
                        pl = JsonSerializer.Deserialize<PlaylistDef>(JSON, _jsonOpts);
                        if (pl == null) continue;
        ...
        private Task PlayLocalPlaylistAsync(PlaylistDef pl, int startIndex)
        {
        ...
        private void HandleSelect(int Selected)
        {
            ...
            {
                case ScreenType.IPOD_NOW_PLAYING:
                    if (Selected == 1)
                    {
                        PushState();
                        OffsetMENU = 0;
                ...
                case ScreenType.IPOD_REQUEST:
                    OffsetMENU = 0;
                    if (Selected == 1) { PushState(); DisplayPlaylists(); }
                ...
                case ScreenType.IPOD_REQUEST_PLAYLIST:
                    {
                        if (selected.Data is string plName && LocalPlaylists.TryGetValue(plName, out var pl))
                        {
                            CurrentLocalPlaylist = pl;
                            MusicDB.CurrentPlaylist = pl.Name;
                            PushState();
                            OffsetMENU = 0;
                            DisplayPlaylistOpts(pl.Name);
                        }
                        break;
                    }

                case ScreenType.IPOD_REQUEST_GENRE:
                ...
                case ScreenType.IPOD_REQUEST_ARTIST:
                    if (!string.IsNullOrEmpty(selected.Text))
                    {
                        PushState();
                        OffsetMENU = 0;
                ...
                case ScreenType.IPOD_REQUEST_ALBUM:
                    if (!string.IsNullOrEmpty(selected.Text))
                    {
                        PushState();
                        OffsetMENU = 0;
                ...
                case ScreenType.IPOD_REQUEST_SONG:
                    {
                ...
                case ScreenType.IPOD_PLAYLIST_OPTIONS:
                    {
                        if (Selected == 1) { PushState(); OffsetMENU = 0; DisplayArtists(); }
                        else if (Selected == 2) { PushState(); OffsetMENU = 0; MusicDB.CurrentArtist = null; DisplayAlbums(null); }
                        else if (Selected == 3) { PushState(); OffsetMENU = 0; CurrentSongScope = SongScope.PlaylistSongs; DisplaySongOpts(null); }
                        else if (Selected == 5)
                        {
                            if (CurrentLocalPlaylist != null)
                            {
                                TaskForget(() => PlayLocalPlaylistAsync(CurrentLocalPlaylist, 0));
                                Console.WriteLine($"[iBridge] Playing playlist: {CurrentLocalPlaylist.Name}");
                            }
                        }
                        break;
                    }

                case ScreenType.IPOD_GENRE_OPTIONS:
                    if (Selected == 1) { PushState(); OffsetMENU = 0; DisplayArtists(); }
                    else if (Selected == 2) { PushState(); OffsetMENU = 0; MusicDB.CurrentArtist = null; DisplayAlbums(null); }
                    else if (Selected == 3) { PushState(); OffsetMENU = 0; MusicDB.CurrentAlbum = null; DisplaySongOpts(null); }
                    else if (Selected == 5) Console.WriteLine($"[iBridge] Playing genre: {selected.Data}");
                    break;

                case ScreenType.IPOD_ARTIST_OPTIONS:
                    if (Selected == 1)
                    {
                        PushState();
                        OffsetMENU = 0;
                    ...
                    else if (Selected == 5)
                    {
                        ...
                        if (!string.IsNullOrEmpty(artist))
                        {
                            ...
                            Console.WriteLine($"[iBridge] Playing: {artist}");
                        }
                    }
                    break;

                case ScreenType.IPOD_ALBUM_OPTIONS:
                    if (Selected == 1)
                    {
                        PushState();
                        OffsetMENU = 0;
                    ...
                    else if (Selected == 5)
                    {
                        ...
                        if (!string.IsNullOrEmpty(album))
                        {
                            ...
                            Console.WriteLine($"[iBridge] Playing album: {album}");
                        }
                    }
                    break;
            }
        ...
        {
            var msg = RNETiBridge.CreateTransition(ActiveController, ActiveZone, Source, (byte)type);
            SendMessage(msg);
            CurrentScreen = type;
            Thread.Sleep(90);
        }
        ...
        private void SendMenu(IEnumerable<string> items)
        {
            ...
            //Console.WriteLine($"[iBridge Menu] screen={CurrentScreen} offset={OffsetMENU} count={padded.Count}");

            for (int i = 1; i <= 7; i++)
            {
                var text = padded[i - 1];
                var msg = RNETiBridge.CreateMenuItemV551(ActiveController, ActiveZone, Source, i, (byte)CurrentScreen, text);

                //Console.WriteLine($"[iBridge Menu] line={i} text='{text}' bytes={msg.Length}");

                Thread.Sleep(25);
                _sm.Write(msg);
                // if sent too fast it just doesnt even get to process the name
                Thread.Sleep(200);
            }
        }
        ...
        private void DisplayNowPlaying()
        {
            ...
            SendScreen(ScreenType.IPOD_NOW_PLAYING);
            SendMenu(new[] { "Music" });
        ...
        {
            SendScreen(ScreenType.IPOD_REQUEST);
            var items = new[] { "by Playlist", "by Genre (NS)", "by Artist", "by Album", "by Song" };
            SendMenu(items);
        ...
        {
           SendScreen(ScreenType.IPOD_REQUEST_PLAYLIST);
        ...
        {
           // SendScreen(ScreenType.IPOD_REQUEST_GENRE);
        ...
        {
            SendScreen(ScreenType.IPOD_REQUEST_ARTIST);
        ...
        {
            // if no artist, assume just getting all albums so return them
        ...
        private void DisplayAlbums(string? artist)
        {
            ...
            SendScreen(ScreenType.IPOD_REQUEST_ALBUM);
        ...
        private void DisplaySongOpts(string? album)
        {
            ...
            SendScreen(ScreenType.IPOD_REQUEST_SONG);
        ...
        private void DisplayPlaylistOpts(string playlist)
        {
            MusicDB.CurrentPlaylist = playlist;
            SendScreen(ScreenType.IPOD_PLAYLIST_OPTIONS);

            var items = new[] { "View Artists", "View Albums", "View Songs", "", playlist };
            SendMenu(items);
        ...
        private void DisplayGenreOpts(string genre)
        {
            ...
            SendScreen(ScreenType.IPOD_GENRE_OPTIONS);
        ...
        {
            MusicDB.CurrentArtist = artist;
            SendScreen(ScreenType.IPOD_ARTIST_OPTIONS);
        ...
        private void DisplayAlbumOpts(string album)
        {
            ...
            SendScreen(ScreenType.IPOD_ALBUM_OPTIONS);
        ...
        public void SendNP(string device, string artist, string album, string title)
        {
            lock (NPLock)
            {
                _sm.Write(RNETiBridge.CreateNP(Source, 0x06, device));
                Thread.Sleep(60);

                _sm.Write(RNETiBridge.CreateNP(Source, 0x82, artist));
                Thread.Sleep(60);

                _sm.Write(RNETiBridge.CreateNP(Source, 0x84, album));
                Thread.Sleep(60);

                _sm.Write(RNETiBridge.CreateNP(Source, 0x81, title));
                Thread.Sleep(80);
            ...
            _ = Task.Run(async () =>
            {
                while (!token.IsCancellationRequested)
                {
                    try
                    {
                        ...
                        var now = st?.Now;
                        Console.WriteLine($"[iBridge] status now={(st?.Now == null ? "null" : st.Now.Artist + " - " + st.Now.Title)}");
                    ...
                    {
                        Console.WriteLine($"[iBridge] NP poll error: {ex.Message}");
                    }
